...
pub const Console = struct {
    ...
    fn write(context: ConsoleContext, bytes: []const u8) ConsoleError!usize {
        ...
        for (bytes) |c| {
            if (c == '\n') {
                ...
                self.writer.write_ascii(8 * self.cur_col, 16 * self.cur_row, c, self.fgc);
    ...
    fn newline(self: *Self) void {
        ...
        if (self.cur_row < kRows - 1) {
            ...
            for (0..16 * kRows) |y| {
                for (0..8 * kCols) |x| {
                    self.writer.write_pixel(@truncate(x), @truncate(y), self.bgc);
                }
            }
            // Scroll up.
            for (0..kRows - 1) |row| {
                @memcpy(&self.buffer[row], &self.buffer[row + 1]);
                self.writer.write_string(0, @truncate(16 * row), &self.buffer[row], self.fgc);
            }
        }
    }
};
